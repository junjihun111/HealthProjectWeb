		...
		<c:forEach items="${sessionScope.listjoin}" var="board" varStatus="cnt">
			...
			<tr>
				<td width=400 height=40 colspan="2" >
					...
					<span id="deletedaglelist${cnt.count}"> <button id="board_delete${cnt.count}">삭제</button></span><c:choose><c:when test="${not empty sessionScope.managerID}">
						<c:when test="${not empty sessionScope.managerPW}">
							<a href="deletedagle.do?page=${cnt.count}&content=${dagle.content}"><input type="button" name="deletemanager" id="deletemanager" value="관리자삭제"><input type="hidden" name="content"></a>
						</c:when>
					</c:when>
				</c:choose>
				</td>
		...
		<script type="text/javascript">
			...
			<c:forEach items="${sessionScope.listjoin}" var="board" varStatus="cnt">
				...
				$("#board_delete${cnt.count}").on("click",function(){
					...
					var txtlire = "<span id=deletedaglelist${cnt.count}> <button id=board_delete${cnt.count}>삭제</button></span>";
				...
				/* <c:forEach items="${sessionScope.listjoin}" var="boardle" varStatus="cnt">
					<c:forEach items="${boardle.daglelist}" var="dagle" varStatus="cnt">
						$("#deletedaglelist${cnt.count}").html(txtlire);
					</c:forEach>
				</c:forEach> */
